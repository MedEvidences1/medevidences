...
const Astrology = () => {
  ...
  useEffect(() => {
    axios.get(`${API}/astrology/channels`).then((res) => setChannels(res.data.channels || {})).catch(console.error);
    search();
  }, []);
  ...
  return (
    <div className="space-y-6" data-testid="astrology-view">
      ...
      <Card className="terminal-card astrology-accent">
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2">
            ...
            VEDIC_ASTROLOGY_CHANNELS
          </CardTitle>
          <CardDescription className="text-[#888]">
            Track predictions from renowned Vedic astrologers for disaster and war forecasting
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid md:grid-cols-3 gap-4">
            {Object.entries(channels).map(([key, channel]) => (
              <div key={key} className="p-4 bg-[#0A0A0A] border border-[#9D4EDD]/30 hover:border-[#9D4EDD] transition-colors">
                <div className="font-medium text-[#EDEDED]">{channel.name}</div>
                <div className="text-xs text-[#888] mt-1">ID: {channel.channel_id}</div>
                <div className="flex flex-wrap gap-1 mt-2">
                  {channel.specialty?.map((s, i) => (
      ...
      <Card className="terminal-card">
        <CardHeader>
          <CardTitle className="text-sm flex items-center gap-2">
            <Play className="w-4 h-4 text-[#9D4EDD]" />
            SEARCH_PREDICTIONS
          </CardTitle>
        </CardHeader>
        <CardContent>
          <div className="flex gap-2 mb-4">
            <Input
              data-testid="astrology-input"
              value={query}
              onChange={(e) => setQuery(e.target.value)}
              className="terminal-input flex-1"
              onKeyDown={(e) => e.key === "Enter" && search()}
            />
            <Button onClick={search} disabled={loading} className="bg-[#9D4EDD] hover:bg-[#9D4EDD]/80 text-white" data-testid="astrology-search-btn">
              {loading ? <RefreshCw className="w-4 h-4 animate-spin" /> : <Search className="w-4 h-4" />}
              <span className="ml-2">SEARCH</span>
            </Button>
          </div>
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {videos.map((v, i) => (
              <div key={i} className="p-4 bg-[#0A0A0A] border border-[#1F1F1F] hover:border-[#9D4EDD] transition-colors">
                <div className="font-medium text-sm text-[#EDEDED] line-clamp-2">{v.title}</div>
                <div className="text-xs text-[#888] mt-1">{v.channel}</div>
                <a href={v.url} target="_blank" rel="noopener noreferrer" className="text-xs text-[#9D4EDD] mt-2 flex items-center gap-1 hover:underline">
